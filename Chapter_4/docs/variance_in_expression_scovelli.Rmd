...
Perhaps do similar co-expression plots for the three tissue types as well (a 3-D plot, maybe?)
...
for(tissue in unique(samples$Organ)){
  ...
  for(sex in unique(samples$Sex)){
    ...
    sex_mean <- rowMeans(log(sex_cnts + .001))
    
    #Calculate the standard error for each row
    sex_sem <- apply(log(sex_cnts + .001), 1, sem)
...
colors <- c(sex_biased="#809FFF75", unbiased="#80808050")
pal<-mako(200,alpha=0.5,direction = -1)
intervals<-findInterval(abs(res$log2FoldChange),
                        seq(min(abs(res$log2FoldChange), na.rm = TRUE),
...
ymax<-max(txpt_sum_stats[,grep("USEM",colnames(txpt_sum_stats))], na.rm = TRUE)

par(mfrow=c(1,3))
for(tissue in unique(samples$tissue)){
 
  out_cols<-grep(substring(tissue,1,3),colnames(txpt_sum_stats), value=TRUE)
  
  plot(txpt_sum_stats[,grep("Fem_Mean", out_cols,value=TRUE)],
       txpt_sum_stats[,grep("Mal_Mean", out_cols,value=TRUE)],
       xlim=c(0,ymax),
       ylim=c(0,ymax),
       xlab="log female gene counts",
       ylab="log male gene counts",
       ...
       pch=19)
  # add bar in female direction
  arrows(x0 = txpt_sum_stats[,grep("Fem_LSEM", out_cols,value=TRUE)],
         y0 = txpt_sum_stats[,grep("Mal_Mean", out_cols,value=TRUE)],
         x1 = txpt_sum_stats[,grep("Fem_USEM", out_cols,value=TRUE)],
         y1 = txpt_sum_stats[,grep("Mal_Mean", out_cols,value=TRUE)],
         length=0,
         col=col_test# colors[txpt_sum_stats$SexBiased])
  )
  # add bar in male direction
  arrows(y0 = txpt_sum_stats[,grep("Mal_LSEM", out_cols,value=TRUE)],
         x0 = txpt_sum_stats[,grep("Fem_Mean", out_cols,value=TRUE)],
         y1 = txpt_sum_stats[,grep("Mal_USEM", out_cols,value=TRUE)],
         x1 = txpt_sum_stats[,grep("Fem_Mean", out_cols,value=TRUE)],
         length=0,
         col=col_test# colors[txpt_sum_stats$SexBiased])
  )
}
...
# 
# plot(c(0, 1), c(0, 1), type='n', axes=F, xlab='', ylab='')  ## ini plot2
par(xpd=TRUE)
