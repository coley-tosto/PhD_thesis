...
SS_blast_files <- list.files(blast_path, pattern = "biased")
...
## Investigating expression of some sex-specific genes
...
```{r plot-panther-SBG-skin}
#Calculate the frequency at which each protein class occurs within the different tissue types
##Create an empty dataset to store the values in
all_go_sums_skin <- data.frame(matrix(ncol = 5,
                                      nrow = 0))

##Add appropriate column names
colnames(all_go_sums_skin) <- c(colnames(all_go_dat),
                                "prop")

##Loop through each bias type to calculate frequencies
for (bias in c("SS_femS_biased_TRgenes_reiro_blast",
               "SS_malS_biased_TRgenes_reiro_blast")) {
  ...
  #Export the data to the previously created dataframe
  all_go_sums_skin <- rbind(tmp, all_go_sums_skin)
  
}


#Change all of the protein classes that have a frequency of less than 0.02 to "Other"
for(class in unique(all_go_sums_skin$Var1)){
  
  #Pull out the rows that contain that protein class
  match_rows <- which(all_go_sums_skin$Var1==class)
  
  #If any of the proportions associated with those rows is less than 0.01, classify as other
  if(any(all_go_sums_skin$prop[match_rows]>0.01)==FALSE){
    
    all_go_sums_skin$Var1[match_rows] <- " other"
    
  }
}

#Change format of "Unclassified so it will come first"
all_go_sums_skin$Var1[all_go_sums_skin$Var1=="Unclassified"] <-  " unclassified"

#write.csv(all_go_sums, "data/GO_freq_SBG.csv", row.names = FALSE)

#Generate the GO plot
bias_col <- c("SS_femS_biased_TRgenes_reiro_blast" = "#7fc97f", 
               "SS_malS_biased_TRgenes_reiro_blast" = "#beaed4")

ggplot(all_go_sums_skin, 
...
##Loop through each bias type to calculate frequencies
for (bias in c("SS_femL_biased_TRgenes_reiro_blast",
               "SS_malL_biased_TRgenes_reiro_blast")) {
...
#Generate the GO plot
bias_col <- c("SS_femL_biased_TRgenes_reiro_blast" = "#7fc97f", 
               "SS_malL_biased_TRgenes_reiro_blast" = "#beaed4")
