...
We can see that the highest similarities (aside from same samples comparissons) is between samples from the same organ (Fig. \@ref(fig:sample-dist)). After that, the next highest similarities can be seen between the male and female gonads. There is one gill sample that appears to be quite different from the rest, sample FLG4M4, definitely a sample to be paying attention to as we move through with the rest of the analysis
...
pcaData <- plotPCA(vsd, intgroup = c("Organ", "Sex"), returnData = TRUE, ntop = nrow(dds_FL))
...
#Pulling the top 50 diff expressed genes in Males
top50_MGill_trin_genes <- cbind(rownames(head(gill_con_res_p05[order(
  gill_con_res_p05$log2FoldChange, decreasing = TRUE),], n=50)))
#Run once to create the file and then commented out
...
top21_FGill_trin_genes <- cbind(rownames(head(gill_con_res_p05[order(
  gill_con_res_p05$log2FoldChange, decreasing = FALSE),], n=21)))
#Run once to create the file and then commented out
...
write.table(as.data.frame(gonad_mal_biased), "gonad_mal_biased.txt", row.names = TRUE)
write.table(as.data.frame(gonad_fem_biased), "gonad_fem_biased.txt", row.names = TRUE)
write.table(as.data.frame(gill_mal_biased), "gill_mal_biased.txt", row.names = TRUE)
write.table(as.data.frame(gill_fem_biased), "gill_fem_biased.txt", row.names = TRUE)
write.table(as.data.frame(liver_mal_biased), "liver_mal_biased.txt", row.names = TRUE)
write.table(as.data.frame(liver_fem_biased), "liver_fem_biased.txt", row.names = TRUE)

gonad_mal_biased <- read.table("data/gonad_mal_biased.txt", row.names = 1)
gonad_fem_biased <- read.table("data/gonad_fem_biased.txt", row.names = 1)
gill_mal_biased <- read.table("data/gill_mal_biased.txt", row.names = 1)
gill_fem_biased <- read.table("data/gill_fem_biased.txt", row.names = 1)
liver_mal_biased <- read.table("data/liver_mal_biased.txt", row.names = 1)
liver_fem_biased <- read.table("data/liver_fem_biased.txt", row.names = 1)
...
for(organ in organs){
  ...
  fem0_organ <- assay(dds_FL)[rowSums(assay(dds_FL)[, dds_FL$Sex == "F" 
                                                    & dds_FL$Organ==organ]) < 1,
                              dds_FL$Organ == organ]
  
  ##Pull out rows where median of male count >=10 for that organ
  mal10_organ <- apply(assay(dds_FL)[, dds_FL$Sex == "M" 
                                     & dds_FL$Organ == organ],
                       1,
  ...
  mal0_organ <- assay(dds_FL)[rowSums(assay(dds_FL)[, dds_FL$Sex == "M" 
                                                    & dds_FL$Organ==organ]) < 1,
                              dds_FL$Organ == organ]
...
logFC_long_noNB <- logFC_long[logFC_long$bias != "UB",]
...
logFC_long_noNB$bias_cat <- ifelse(abs(logFC_long_noNB$logFC) >= 2 & abs(logFC_long_noNB$logFC) < 3, 
                                   biased_bins[1],
                                   ifelse(abs(logFC_long_noNB$logFC) >= 3 & abs(logFC_long_noNB$logFC) < 5, 
                                          biased_bins[2],
                                          ifelse(abs(logFC_long_noNB$logFC) >= 5 & abs(logFC_long_noNB$logFC) < 10,
                                                 biased_bins[3], 
                                                 biased_bins[4])
                                          )
                                   )
...
bias_cat_gill <- cbind(bias_cat_table[,, "Gill"], 
                       "Sex-specific" = c(nrow(FL_sex_specific_genes$Gill_female_specific), nrow(FL_sex_specific_genes$Gill_male_specific)))
write.table(bias_cat_gill, "data/bias_cat_gills.txt", row.names = TRUE)

bias_cat_gonad <- cbind(bias_cat_table[,, "Gonad"], 
                        "Sex-specific" = c(nrow(FL_sex_specific_genes$Gonad_female_specific), nrow(FL_sex_specific_genes$Gonad_male_specific)))
write.table(bias_cat_gonad, "data/bias_cat_gonad.txt", row.names = TRUE)

bias_cat_liver <- cbind(bias_cat_table[,, "Liver"], "Sex-specific" = c(nrow(FL_sex_specific_genes$Liver_female_specific), nrow(FL_sex_specific_genes$Liver_male_specific)))
write.table(bias_cat_liver, "data/bias_cat_liver.txt", row.names = TRUE)

#Plot the counts for each tissue type
par(mfrow=c(1, 3))
barplot(bias_cat_gill[,biased_bins], beside = TRUE, ylim = c(0, max(bias_cat_table) + 1), col = c("#7fc97f", "#beaed4"),
        main = "Gill", ylab = "Number of Genes", xlab = "Bias Category")
barplot(bias_cat_gonad[,biased_bins], beside = TRUE, ylim = c(0, max(bias_cat_table) + 1), col = c("#7fc97f", "#beaed4"),
        main = "Gonad")
barplot(bias_cat_liver[,biased_bins], beside = TRUE, ylim = c(0, max(bias_cat_table) + 1), col = c("#7fc97f", "#beaed4"),
        main = "Liver")
...
```{r, eval=FALSE}
write.table(c(blast_output_merged$FL_femG_biased_TRgenes_blast$gene_name,
              blast_output_merged$FL_femGon_biased_TRgenes_blast$gene_name,
              blast_output_merged$FL_femL_biased_TRgenes_blast$gene_name,
              blast_output_merged$FL_malG_biased_TRgenes_blast$gene_name,
              blast_output_merged$FL_malGon_biased_TRgenes_blast$gene_name,
              blast_output_merged$FL_malL_biased_TRgenes_blast$gene_name),
           'FL_GOnames.txt', 
            sep = " ", 
            quote=FALSE, 
            row.names = FALSE, 
...
write.table(rbind(blast_output_merged$FL_femG_biased_TRgenes_blast
                  [,c("gene_name", "log2FoldChange")],
...
all_go_dat$freq_organ <- 0
length(unique(all_go_dat[all_go_dat$tissue == "Gill",]$Var1))

# make a barplot somehow
my_colors <- c("Gonad" = "#EEB422", "Liver" = "#EE8262", "Gill" = "#20B2AA")
ggplot(all_go_dat, 
       aes(Var1, prop, fill = tissue)) +   
...
```{r fig-2, fig.height=11, fig.width=7}
p1 <- ggdraw() + draw_image("imgs/FCvbiasFL.png")
p2 <- ggdraw() + draw_image("imgs/bias_bins_FL.png")
p3 <- ggdraw() + draw_image("imgs/FL_upsetall.png")
plot_grid(p1,
          NULL,
          p2,
          NULL,
