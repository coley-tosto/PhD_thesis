...
#Filtering the dds object to get rid of the DESeq outliers
dds_FL_filtered <- dds_FL[!(rownames(dds_FL) %in% out_ids), ]

dds_FL_filtered$Organ <- ifelse(dds_FL_filtered$Organ =="Ovaries",
                        paste0("Gonad"),
                        ifelse(dds_FL_filtered$Organ == "Testis",
                               paste0("Gonad"),
                               paste0(dds_FL_filtered$Organ))
                        )
dds_FL_filtered$Organ <- as.factor(dds_FL_filtered$Organ)

#Create a vector with the different organ types
organs <- levels(colData(dds_FL_filtered)$Organ)
...
for(organ in organs){
  ...
  fem0_organ <- assay(dds_FL_filtered)[rowSums(assay(dds_FL_filtered)[, dds_FL_filtered$Sex == "F" 
                                                    & dds_FL_filtered$Organ==organ]) < 1,
                              dds_FL_filtered$Organ == organ]
  
  ##Pull out rows where median of male count >=10 for that organ
  mal10_organ <- apply(assay(dds_FL_filtered)[, dds_FL_filtered$Sex == "M" 
                                     & dds_FL_filtered$Organ == organ],
                       1,
  ...
  mal0_organ <- assay(dds_FL_filtered)[rowSums(assay(dds_FL_filtered)[, dds_FL_filtered$Sex == "M" 
                                                    & dds_FL_filtered$Organ==organ]) < 1,
                              dds_FL_filtered$Organ == organ]
  
  ##Pull out rows where median of female count >=10 for that organ
  fem10_organ <- apply(assay(dds_FL_filtered)[, dds_FL_filtered$Sex == "F" & dds_FL_filtered$Organ == organ],
                       1,
                       function(x) median(x) >=10)
...
for (organ in organs) {
  ...
  tmp <- as.data.frame(assay(dds_FL_filtered)[, dds_FL_filtered$Organ == organ])
  ...
  tmp$Fmedian <- apply(tmp[, c(colnames(tmp_F))], 1, median)
  tmp$Mmedian <- apply(tmp[, c(colnames(tmp_M))], 1, median)
...
write.csv(logFC_long_all, "logFC_long_taubias_SS.csv", row.names = FALSE)
