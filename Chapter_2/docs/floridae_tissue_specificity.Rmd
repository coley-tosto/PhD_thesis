...
for(i in 1:length(FL_sex_specific_genes)){
  ...
  ssp<-do.call(rbind,apply(tmp, 1, function(one_gene){
    
    if(one_gene["geneID"] %in% 
       logFC_long_all$geneID[logFC_long_all$tissue == organs[[i]] &
                             logFC_long_all$bias == bias[[i]]]){
      
      one_gene_dat<-logFC_long_all[logFC_long_all$geneID == one_gene["geneID"] &
  ...
  rownames(ssp)<-NULL
  logFC_long_all<-rbind(logFC_long_all, ssp)
  browser()
}
```
