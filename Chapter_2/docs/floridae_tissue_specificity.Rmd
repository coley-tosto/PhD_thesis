...
#Looking at the TPMS
tpms[rownames(tpms)== top_gonad$Row.names[1],] #Testis-specific
tpms[rownames(tpms)== top_gonad$Row.names[2],] #Liver-specific
tpms[rownames(tpms)== top_gonad$Row.names[3],] #Testis-specific
tpms[rownames(tpms)== top_gonad$Row.names[4],] #Testis-specific
tpms[rownames(tpms)== top_gonad$Row.names[5],] #Ovary-specific
tpms[rownames(tpms)== top_gonad$Row.names[6],] #Testis-specific
tpms[rownames(tpms)== top_gonad$Row.names[7],] #Ovary-specific
tpms[rownames(tpms)== top_gonad$Row.names[8],] #Ovary-specific
tpms[rownames(tpms)== top_gonad$Row.names[9],] #Liver-specific
tpms[rownames(tpms)== top_gonad$Row.names[10],] #Testis-specific
tpms[rownames(tpms)== top_gonad$Row.names[11],] #Ovary-specific
tpms[rownames(tpms)== top_gonad$Row.names[12],] #Ovary-specific
tpms[rownames(tpms)== top_gonad$Row.names[13],] #Liver-specific
...
#write.csv(logFC_long_all, "logFC_long_taubias.csv", row.names = FALSE)
```
...
for(organ in organs){
  ...
  fem0_organ_names <- which(rowSums(t(apply(counts(dds_FL_exp_filtered, normalized = TRUE)[, dds_FL_exp_filtered$Sex == "F" & 
                                                                                          dds_FL_exp_filtered$Organ == organ],
                                      1,
                                      function(x) x <= 10)
                                )
                              ) == ncol(counts(dds_FL_exp_filtered, normalized = TRUE)[, dds_FL_exp_filtered$Sex == "F" & 
                                                                                dds_FL_exp_filtered$Organ == organ])
                      )
  
  fem0_organ <- counts(dds_FL_exp_filtered, normalized = TRUE)[rownames(counts(dds_FL_exp_filtered, normalized = TRUE)) %in% 
                                                        names(fem0_organ_names), 
                                                      dds_FL_exp_filtered$Organ == organ]
  
  ##Pull out rows where median of male count >=20 for that organ
  mal10_organ <- apply(counts(dds_FL_exp_filtered, normalized = TRUE)[, dds_FL_exp_filtered$Sex == "M" 
                                     & dds_FL_exp_filtered$Organ == organ],
                       1,
  ...
  mal0_organ_names <- which(rowSums(t(apply(counts(dds_FL_exp_filtered, normalized = TRUE)[, dds_FL_exp_filtered$Sex == "M" & 
                                                                                    dds_FL_exp_filtered$Organ == organ],
                                      1,
                                      function(x) x <= 10)
                                )
                              ) == ncol(counts(dds_FL_exp_filtered, normalized = TRUE)[, dds_FL_exp_filtered$Sex == "M" & 
                                                                                dds_FL_exp_filtered$Organ == organ])
                      )
  
  mal0_organ <- counts(dds_FL_exp_filtered, normalized = TRUE)[rownames(counts(dds_FL_exp_filtered, normalized = TRUE)) %in% 
                                                        names(mal0_organ_names), 
                                                      dds_FL_exp_filtered$Organ == organ]
  
  ##Pull out rows where median of female count >=10 for that organ
  fem10_organ <- apply(counts(dds_FL_exp_filtered, normalized = TRUE)[, dds_FL_exp_filtered$Sex == "F" &
                                                                        dds_FL_exp_filtered$Organ == organ],
                       1,
                       function(x) median(x) >=20)
...
for (organ in organs) {
  ...
  browser()
  tmp <- as.data.frame(counts(dds_FL_exp, normalized = TRUE)[, dds_FL_exp$Organ == "Gill"])
...
my_colors <- c("Gonad" = "#EEB42275", "Liver" = "#EE826275", "Gill" = "#20B2AA75")
tauvbias <- ggplot(logFC_long_all, aes(x = sqrt(abs(logFC)), y = tau, color = tissue)) +
  geom_point() +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black")) +
  geom_smooth(data = logFC_long_all[logFC_long_all$tissue == "Gonad",], 
              method = "lm", color = "#EEB422", se = FALSE, lty = 1) +
  geom_smooth(data = logFC_long_all[logFC_long_all$tissue == "Liver",], 
              method = "lm", color = "#EE7073", lty = 1, se = FALSE) +
  geom_smooth(data = logFC_long_all[logFC_long_all$tissue == "Gill",], 
              method = "lm", color = "#20B2AA", se = FALSE, lty = 1) + 
  labs(x="Sex Bias (|Log2 Fold Change|)", y=expression(tau["TPM"])) +
  guides(fill = guide_legend(title = "Organ", order = 3))

p2 <- ggdraw() + draw_image("imgs/tau_bias_cat_FL.jpg")
plot_grid(tauvbias, p2,
          ncol = 1,
          align = 'v',
          labels = c("A", "B"))
```
