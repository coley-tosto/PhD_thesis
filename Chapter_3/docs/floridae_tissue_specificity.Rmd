...
Once all the TPMs were gathered for the different samples I filtered some of the rows out based on results of the differential expression analysis done by DESeq2. The two filtering steps applied to the TPM dataset included:
  ...
  1. Keeping only the rows that weren't filtered out in the DESeq2 dataset due to low counts (rowSum $\le$ 10) or really high expression (rowSum $>$ 1,000,000).
  
  2. Removing the rows that corresponded to genes that were "outliers" in the DESeq2 analysis.

It should be noted that the DESeq2 filtering was done based off of the count data and not the TPMs, but as TPMs are just normalized counts they should be correlated and something that was had low gene counts should also have a low TPM and anything that was considered an outlier in the counts could also be an outlier in terms of TPM, but it may not be perfect. The sample FLG4M4 was also removed from this dataset the same way it was removed in the differential expression analysis.
...
#write.csv(liver_bias_tau_all, "liver_bias_tau.csv", row.names = FALSE)
...
#write.csv(gill_bias_tau_all, "gill_bias_tau.csv", row.names = FALSE)
...
```{r tau-v-biascat-plot-all-vp-jitt, echo=FALSE, fig.height=8, fig.cap="Violin plot showing tissue specifcity (tau) across the different bias levels. Color denotes female-biased versus male biased versus unbiased and jitters were added to show all of the raw tau values."}
my_colors <- c("FB" = "#7fc97f", "MB" = "#beaed4", "UB" = "darkgray" )
...
# Re-running tau with "Gonads" instead of "Ovaries and Testis"
```{r reset-colData}
colData$Organ <- as.factor(ifelse(colData$Organ == "Ovaries", 
                                  paste0("Gonad"),
                                  ifelse(colData$Organ == "Testis",
                                         paste0("Gonad"),
                                         paste0(colData$Organ))
                                  )
                           )
...
```{r calcTau_gonads}
tau_gonads <- apply(tpms, 1, est_tau, colDat=colData)

tau_fem_gonads <- apply(tpms[,which(colData$Sex=="F")], 1, est_tau,
                        colData[which(colData$Sex=="F"),])

tau_mal_gonads <- apply(tpms[,which(colData$Sex=="M")], 1, est_tau,
                        colData[which(colData$Sex=="M"),])

```

```{r tauHist_gonads,fig.cap="Distribution of tissue specificity estimates for only the male samples (left) versus only the female samples (middle) versus the male samples and the female samples (right).", echo=FALSE}
par(mfrow=c(1,3))
hist(tau_mal_gonads,
     col="grey",
     xlab=expression(tau),
...
```{r tau-v-FC-gonads, echo=FALSE, fig.cap="Sex bias (in terms of the absolute value of the log2FoldChange) versus tissue specificity (tau) for all genes that are signifcantly expressed in the gonads, liver, and gills."}
#Merge the noNA datasets that include all of the sex-biased and non-sex-biased genes with the tau datasets
liver_bias_tau_all_gonads <- merge(as.data.frame(liver_con_res_noNA), as.data.frame(tau_gonads), by = "row.names")
#write.csv(liver_bias_tau_all, "liver_bias_tau.csv", row.names = FALSE)

gonad_bias_tau_all_gonads <- merge(as.data.frame(gonad_con_res_noNA), as.data.frame(tau_gonads), by = "row.names")
#write.csv(gonad_bias_tau_all, "gonad_bias_tau.csv", row.names = FALSE)

gill_bias_tau_all_gonads <- merge(as.data.frame(gill_con_res_noNA), as.data.frame(tau_gonads), by = "row.names")
#write.csv(gill_bias_tau_all, "gill_bias_tau.csv", row.names = FALSE)


#Plot logFC against Tau for all of the organs
plot(sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)[sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)) < 4]),
     gonad_bias_tau_all_gonads$tau[sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)) < 4],
     pch=19,
     col = "#EEB42275",
     xlab="sex bias (|log2 fold change|)",
     ylab=expression(tau["TPM"]),
     xlim = c(0, 7),
     ylim = c(0, 1))
points(sqrt(abs(liver_bias_tau_all_gonads$log2FoldChange)),
     liver_bias_tau_all_gonads$tau,
     pch=19,
     col="#EE826275")
points(sqrt(abs(gill_bias_tau_all_gonads$log2FoldChange)),
     gill_bias_tau_all_gonads$tau,
     pch=19,
     col = "#6E8B3D75")

#Add the best fit lines
abline(lm(gonad_bias_tau_all_gonads$tau[sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)) < 4] ~ sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange))[sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)) < 4]), 
       col="#EEB422", lwd = 3)
abline(lm(liver_bias_tau_all_gonads$tau ~ sqrt(abs(liver_bias_tau_all_gonads$log2FoldChange))), 
       col = "#EE8262", lwd = 3, lty = 2)
abline(lm(gill_bias_tau_all_gonads$tau ~ sqrt(abs(gill_bias_tau_all_gonads$log2FoldChange))), 
       col = "#6E8B3D", lwd = 3, lty = 3)
...
```{r tau-v-FC-cor-gonads, echo=FALSE, warning=FALSE}
cor.test(gonad_bias_tau_all_gonads$tau[sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)) < 4], 
         abs(gonad_bias_tau_all_gonads$log2FoldChange)[sqrt(abs(gonad_bias_tau_all_gonads$log2FoldChange)) < 4], 
         method = "spearman")
cor.test(liver_bias_tau_all_gonads$tau, 
         abs(liver_bias_tau_all_gonads$log2FoldChange), 
         method = "spearman")
cor.test(gill_bias_tau_all_gonads$tau, 
         abs(gill_bias_tau_all_gonads$log2FoldChange), 
         method = "spearman")
...
```{r}
logFC_long_all_gonads <- merge(logFC_long_all, as.data.frame(tau_gonads), by.x = "geneID", by.y = "row.names")

my_colors <- c("FB" = "#7fc97f", "MB" = "#beaed4", "UB" = "darkgray" )
logFC_long_all_gonads$bias_cat <- factor(logFC_long_all_gonads$bias_cat,
                                         levels = biased_bins, ordered = TRUE)
logFC_long_all_gonads$tissue <- factor(logFC_long_all_gonads$tissue, 
                                       levels = c("Gill", "Gonad", "Liver"),
                                       ordered = TRUE)

logFC_long_all_gonads %>%
  ggplot(aes(x = bias_cat, y = tau_gonads, fill = bias)) +
    geom_violin(position = position_dodge(), draw_quantiles = c(0.5)) +
    ...
    labs(x="Bias Category", y=expression(tau["TPM"]))  +
    stat_n_text(data = logFC_long_all_gonads[logFC_long_all_gonads$bias == "FB",],
                aes(x=bias_cat, y=tau_gonads),
                hjust = 1.2,
                #vjust = -2,
                y.pos = 0.95) +
    stat_n_text(data = logFC_long_all_gonads[logFC_long_all_gonads$bias == "MB",],
                aes(x=bias_cat, y=tau_gonads),
                hjust = -0.2,
                y.pos = 0,
                vjust = 2) +
    stat_n_text(data = logFC_long_all_gonads[logFC_long_all_gonads$bias == "UB",],
                aes(x=bias_cat, y=tau_gonads),
                vjust = -4 ) +
    guides(fill = guide_legend(title = "Bias Level", order = 3))
...
```{r tau-v-FC-MFbias, echo=FALSE, fig.cap="Sex bias (in terms of the absolute value of the log2FoldChange) versus tissue specificity (tau) for all genes that are signifcantly expressed. The fold change in this plot  is a difference bewteen males and females regardless of tissue type.", message=FALSE}
#Merge the noNA dataset with the tau datasets
MFbias_tau_all_gonads <- merge(as.data.frame(sex_con_res_noNA), as.data.frame(tau_gonads), by = "row.names")

#Plot logFC against Tau for all of the organs
plot(sqrt(abs(MFbias_tau_all_gonads$log2FoldChange)),
     MFbias_tau_all_gonads$tau,
     pch=19,
     col = "#20B2AA75",
     xlab="sex bias (log2 fold change)",
     ylab=expression(tau["TPM"]))
...
#Add the best fit line
abline(lm(MFbias_tau_all_gonads$tau ~ sqrt(abs(MFbias_tau_all_gonads$log2FoldChange))),
       col="black", lwd = 3, lty = 2)

cor.test(MFbias_tau_all_gonads$tau, 
         abs(MFbias_tau_all_gonads$log2FoldChange), 
         method = "spearman")
```