...
keep <- rowSums(counts(dds_FU)) >= 10 & rowSums(counts(dds_FU)) < 1e6
dds_FU <- dds_FU[keep, ]
...
par(mfrow=c(3, 3))
plotCounts(dds_FU, gene= names(top_tau[1]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[2]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[3]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[4]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[5]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[6]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[7]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[8]), intgroup="Organ")
plotCounts(dds_FU, gene= names(top_tau[9]), intgroup="Organ")
```
...
The absolute value of the log2 fold change was square root transformed. There appears to be a relationship between $\tau$ and sex bias for the gonads (Fig. \@ref(fig:tau-v-bias)), but potentially not for the liver and the gills. This may be due to a low number of sex-biased genes. We can look further into this relationship with a Spearman's rank correlation test. This was done between $\tau$ and the $|log2FoldChange|$ for each organ separately.
...
Surprisingly, only the liver shows a significant correlation. This isn't super obvious from just looking at the plot.
...
For most of the tissues genes that are highly sex-biased in that organ are also tissue-specific in that organ. In the liver, the majority of the liver-specific genes have high expression in one or two liver samples and an expression level of 0 in the rest of the samples.

For the liver, I want to look at some of the samples that are showing a very high sex bias (high logFC), but have lower $\tau$ values:

```{r explore-tau-sb-liver}
##Looking at genes that have a high bias and low tau in the liver
bot_liver <- liver_bias_tau_all[sqrt(abs(liver_bias_tau_all$log2FoldChange)) > 4 & liver_bias_tau_all$tau < 0.5, ]

#Looking at the TPMS
#All had a TPM of 0 in every sample EXCEPT FUL11M2 and FUL12M1
tpms[rownames(tpms)== bot_liver$Row.names[1],] 
tpms[rownames(tpms)== bot_liver$Row.names[2],] 
tpms[rownames(tpms)== bot_liver$Row.names[3],] 
tpms[rownames(tpms)== bot_liver$Row.names[4],] 
tpms[rownames(tpms)== bot_liver$Row.names[5],] 
tpms[rownames(tpms)== bot_liver$Row.names[6],] 
 tpms[rownames(tpms)== bot_liver$Row.names[8],] 
tpms[rownames(tpms)== bot_liver$Row.names[9],] 
tpms[rownames(tpms)== bot_liver$Row.names[10],]
tpms[rownames(tpms)== bot_liver$Row.names[11],]
tpms[rownames(tpms)== bot_liver$Row.names[12],]
tpms[rownames(tpms)== bot_liver$Row.names[13],]
tpms[rownames(tpms)== bot_liver$Row.names[14],]
tpms[rownames(tpms)== bot_liver$Row.names[15],]
tpms[rownames(tpms)== bot_liver$Row.names[16],]
tpms[rownames(tpms)== bot_liver$Row.names[17],]
tpms[rownames(tpms)== bot_liver$Row.names[18],]
tpms[rownames(tpms)== bot_liver$Row.names[19],]
tpms[rownames(tpms)== bot_liver$Row.names[20],]
tpms[rownames(tpms)== bot_liver$Row.names[21],]
tpms[rownames(tpms)== bot_liver$Row.names[22],]
tpms[rownames(tpms)== bot_liver$Row.names[24],]
tpms[rownames(tpms)== bot_liver$Row.names[25],]
tpms[rownames(tpms)== bot_liver$Row.names[26],]
tpms[rownames(tpms)== bot_liver$Row.names[27],]


tpms[rownames(tpms)== bot_liver$Row.names[7],]

tpms[rownames(tpms)== bot_liver$Row.names[23],]

```

Most of the genes looked at above had a TPM of 0 in every sample aside from two of the liver samples, and those two samples were always FUL11M2 and FUL12M1.
